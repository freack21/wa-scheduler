...
router.post('/schedule', async (req, res) => {
    try {
        ...
        const schedule = await schedulerService.createSchedule(req.user.id, number, message, time);
...
router.delete('/schedule/:id', async (req, res) => {
    try {
        const deleted = await schedulerService.deleteSchedule(req.user.id, req.params.id);
        if (deleted) {
...
router.post('/send-video', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error sending video', error: error.message });
    }
});
...
router.post('/send-document', async (req, res) => {
    try {
        ...
        const { default: waService } = await import('../services/WaService.js');
        await waService.sendDocument(req.user.id, number, docUrl, filename, caption);
        res.json({ message: 'Document sent successfully' });
    } catch (error) {
        res.status(500).json({ message: 'Error sending document', error: error.message });
    }
});
...
router.post('/send-sticker', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error sending sticker', error: error.message });
    }
});
